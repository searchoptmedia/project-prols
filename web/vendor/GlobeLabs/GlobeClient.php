...
namespace Vendor\GlobeLabs;

use Propelrr\CoreBundle\APIutils\GlobeLabs\GlobeSms;
use Propelrr\CoreBundle\APIutils\GlobeLabs\GlobeAuth;
use Propelrr\CoreBundle\APIutils\GlobeLabs\GlobePayment;
...
class GlobeClient
{
    ...
    protected function _curlPost($url, $fields = array()) {
        ...
        if (array_key_exists(0, $temp)) {
            ...
            $header = 'API Connection error. Header does not exist.';
        }
        
        if (array_key_exists(1, $temp)) {
            $body = $temp[1];
        } else {
            $body = array('message' => 'API Connection error. Response body does not exist.');
        }
